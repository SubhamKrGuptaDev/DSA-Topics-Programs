 ...
 * node B[i][1].
 *
 * Find whether the graph contains a cycle or not, return 1
 * if cycle is present else return 0.
 *
 * NOTE:
 *
...
public class Cycle_in_Directed_Graph {
    ...
    public int solve(int A, ArrayList<ArrayList<Integer>> B) {
        ...
        for(int i=1; i<A; i++) {
            if(!visited[i]) {
                int temp = dfs(i, adj, visited, path);
                if(temp == 1) return 1;
            }
        }
    ...
    int dfs(int node, ArrayList<ArrayList<Integer>> adj, boolean[] visited, boolean[] path) {
        visited[node] = true;
        ...
        for(int x : adj.get(node)) {
            ...
            if(!visited[x] && dfs(x, adj, visited, path) == 1) {
                return 1;
            }
        }
        path[node] = false;
