...
public class Free_Cars {
    ...
    public int solve(ArrayList<Integer> A, ArrayList<Integer> B) {
        ...
        long totalProfit = 0L;
        for(int i=0; i<n; i++) {
            if(curTime >= list.get(i).time) {
                if(list.get(i).profit > pq.peek()) {
                    int value = pq.poll();
                    totalProfit -= value;
        ...
        return (int)(totalProfit % (1000 * 1000 * 1000 + 7));
    }
    ...
    class Pair {
        ...
        public Pair(Integer time, Integer profit) {
            this.time = time;
            this.profit = profit;
        }
    }
