 ...
 *
 * Note: You can reorder the position of elements within the subset to find the value of
 * the magic number.
 *
 * For Ex:-
 * subset 1 = {1, 5, 1},
 ...
 * number modulo 10^9 + 7, B[1] = minimum possible value of a Magic number modulo 10^9 + 7.
 *
 *
 *
 * Problem Constraints
 ...
 * Magic number % 10^9 + 7,B[1] = minimum possible value of a Magic number % 10^9 + 7.
 *
 *
 *
 * * Example Input *
...
public class Maximum_and_Minimum_Magic {
    public ArrayList<Integer> solve(ArrayList<Integer> A) {
        ...
        while(leftSub < rightSub && rightSub < n) {
            // Max SubSet
            maxSub += Math.abs(A.get(leftSub) - A.get(rightSub));
        ...
        for(int i=1; i<n; i+=2) {
            // Min SubSet
            minSub += Math.abs(A.get(i) - A.get(i-1));
        }


        maxSub = maxSub % mod;
        minSub = minSub % mod;
        result.add((int)maxSub);
        result.add((int)minSub);
