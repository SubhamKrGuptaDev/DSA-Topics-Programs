 ...
 *
 * A person moves to and fro, changing the states of the doors as follows: the person
 * opens a door that is already closed and closes a door that is already opened.
 *
 ...
 * Input 1:
 *
 *  In the first go, he/she alters the states of doors numbered 1, 2, 3, 4, 5. Now,
 *  all doors are open.
 ...
 * Input 2:
 *
 *  In the first go, he/she alters the states of doors numbered 1, 2, 3, 4, 5, 6. Now,
 *  all doors are open.
...
public class Number_Of_Open_Doors {
    ...
    // Optimize Solution count of perfact square root
    public int optimizeApproach(int A) {
        int count = 0;
        for(int i=1; i<=A; i++) {
            if(findPerfactSquare(i)) {
                count++;
            }
        }
    ...
    private boolean findPerfactSquare(int n) {
        for(int i=1; i*i<=n; i++) {
    ...
    // Brute Force
    public int bruteForceApproach(int A) {
        if(A == 1) return 1;
        boolean[] number = new boolean[A];
        for(int i=1; i<A; i++) {
            for(int j=i+1; j<=A; j+=i+1) {
